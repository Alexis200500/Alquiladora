      ...
      <div class="card my-4">
        ...
        <div class="card-body px-0 pb-2">
                  ...
                  <td>
                    <div class="d-flex px-2 py-1">
                      <div class="d-flex flex-column justify-content-center">
                        <h4 class="mb-0 text-sm">{{$puesto->id_puesto}}</h4>
                      </div>
                    </div>
                  </td>
                  <td>
                    <p class="text-ms font-weight-bold mb-0">{{$puesto->puesto}}</p>
                  </td>
                  <td class="align-middle text-center text-sm">
                    @if($puesto->deleted_at)
                      <a href="{{route('activar_puesto',[$puesto->id_puesto])}}">
                        <button class="badge badge-sm bg-gradient-success">Activar</button>
                      </a>
                      <a href="{{route('eliminar_puesto',[$puesto->id_puesto])}}">
                        <button class="badge badge-sm bg-gradient-danger">Eliminar</button>
                      </a>
                    @else
                    <a href="{{route('editar_puesto',[$puesto->id_puesto])}}">
                      <button class="badge badge-sm bg-gradient-info">
                        Editar
                      </button>
                    </a>
                    <a href="{{route('desactivar_puesto',[$puesto->id_puesto])}}">
          ...
          <div class="container"> <br><br>
            <form action="{{route('Puestosimportar')}}" method="post" enctype="multipart/form-data">
              @csrf
            <div class="row">
