      ...
      <div class="card my-4">
        ...
        <div class="card-body px-0 pb-2">
                  ...
                  <td>
                    <div class="d-flex px-2 py-1">
                      <div class="d-flex flex-column justify-content-center">
                        <h4 class="mb-0 text-sm">{{$evento->id_evento}}</h4>
                      </div>
                    </div>
                  </td>
                  <td>
                    <p class="text-ms font-weight-bold mb-0">{{$evento->evento}}</p>
                  </td>
                  <td class="align-middle text-center text-sm">
                    @if($evento->deleted_at)
                      <a href="{{route('activar_evento',[$evento->id_evento])}}">
                        <button class="badge badge-sm bg-gradient-success">Activar</button>
                      </a>
                      <a href="{{route('eliminar_evento',[$evento->id_evento])}}">
                        <button class="badge badge-sm bg-gradient-danger">Eliminar</button>
                      </a>
                    @else
                    <a href="{{route('editar_evento',[$evento->id_evento])}}">
                      <button class="badge badge-sm bg-gradient-info">Editar</button>
                    </a>
                    <a href="{{route('desactivar_evento',[$evento->id_evento])}}">
          ...
          <div class="container"> <br><br>
            <form action="{{route('EventosImportar')}}" method="post" enctype="multipart/form-data">
              @csrf
            <div class="row">
